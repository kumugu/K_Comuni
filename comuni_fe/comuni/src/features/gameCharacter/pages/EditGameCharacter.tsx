...
import { getGameCharacter, updateGameCharacter, getAllGameCharacters } from "../../../shared/api/gameCharacterService.js";
...
const EditGameCharacter = () => {
  const [gameCharacterId, setGameCharacterId] = useState("");
  ...
  const [gameCharacterList, setGameCharacterList] = useState<{ id: string; gameCharacterName: string }[]>([]);
  ...
  useEffect(() => {
    const fetchAllCharacters = async () => {
      try {
        const gameCharacters = await getAllGameCharacters();
        setGameCharacterList(gameCharacters);
      } catch (error) {
        toast.error("캐릭터 목록 불러오기 실패");
      }
    };
    fetchAllCharacters();
  }, []);
  ...
  const handleCharacterSelect = async (id: string) => {
    setGameCharacterId(id);
    try {
      const character = await getGameCharacter(id);
      setGameCharacter(character);
    } catch (error) {
      toast.error("캐릭터 정보를 가져오는 데 실패했습니다.");
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      await updateGameCharacter(gameCharacterId, gameCharacter);
  ...
  return (
   <div className="container mx-auto p-4">
    <h1 className="text-2xl font-bold mb-4">Edit Your Character</h1>
    <div className="mb-4">
      <Label htmlFor="character-select">Select a Character to Edit</Label>
      <select
        id="character-select"
        value={gameCharacterId}
        onChange={(e) => handleCharacterSelect(e.target.value)}
        className="w-full p-2 border rounded"
      >
        <option value="">-- Select Character --</option>
        {gameCharacterList.map((gameCharacter) => (
          <option key={gameCharacter.id} value={gameCharacter.id}>
        ...
        {gameCharacterId && (
          <div className="grid gird-cols-1 md:grid-cols-2 gap-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {["strength", "health", "intelligence", "spirit", "luck"].map((stat) => (
              <div key={stat}>
